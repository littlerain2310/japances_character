...
def main():
    ...
    arg('--batch-size', default=1, type=int)
    ...
    arg('--score-threshold', type=float, default=0.25)
    ...
    root = TRAIN_ROOT
    if args.submission:
        df_valid = pd.read_csv(DATA_ROOT / 'test.csv')
        df_valid['labels'].fillna(value='', inplace=True)
        # df_valid['labels'] = ''
        root = TEST_ROOT
    dataset = Dataset(
